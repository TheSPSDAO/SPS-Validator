            ...
            <Card className="col-span-full dark:bg-gray-800 dark:text-gray-300 dark:shadow-none">
                <CardBody>
                    ...
                    <div className="mt-4 flex 2xl:max-w-96 2xl:w-1/4 lg:w-2/3 md:w-full">
                        <Select label="Token" value={token} onChange={(val) => setToken(val ?? tokens[0])} className="dark:bg-gray-300 dark:text-gray-800" >
                            {tokens.map((token) => (
                                <Option key={token} value={token} className="transition-colors delay-300 duration-500 dark:text-gray-300">
                                    {token}
                                </Option>
                            ))}
                        </Select>
