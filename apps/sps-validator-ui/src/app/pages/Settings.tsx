import { Button, Card, CardBody, CardFooter, Input, Typography, Switch } from '@material-tailwind/react';
...
export function DarkModeToggle() {
    ...
    return (
        <div className="flex flex-row content-between">
            <Typography variant="small" className="dark:text-gray-300 mr-2">
                Light
            </Typography>
            <div className="relative h-6 w-11 flex items-center">
                ...
                />
                <SunIcon className={`absolute left-1 w-4 h-4 text-white dark:text-gray-800 transition-opacity pointer-events-none ${darkMode ? 'opacity-40' : 'opacity-100'}`} />
                <MoonIcon className={`absolute right-1 w-4 h-4 text-black dark:text-gray-300 transition-opacity pointer-events-none ${darkMode ? 'opacity-100' : 'opacity-20'}`} />
            </div>
            <Typography variant="small" className="dark:text-gray-300 ml-2">
                Dark
            </Typography>
        </div>
    );
...
export function Settings() {
    ...
    }, [setAccount, setApi, setPrefix, workingAccount, workingApi, workingPrefix]);
            ...
            <Card className="2xl:w-1/3 lg:w-2/3 sm:w-full dark:bg-gray-800 dark:text-gray-300 dark:shadow-none">
                <CardBody>
                    ...
                    <Typography variant="paragraph" className="dark:text-gray-300">
                        Adjust the theme for the UI.
                    </Typography>
                    <div className="border-gray-400 border-b-[1px] my-5 pb-5">
                        <DarkModeToggle />
                    </div>
                    <Typography variant="paragraph" className="dark:text-gray-300 mt-5">
                        Adjust the settings that the UI uses to interact with the SPS Validator Network.
                    </Typography>
