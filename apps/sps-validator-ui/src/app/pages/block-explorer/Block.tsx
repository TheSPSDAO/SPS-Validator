            ...
            <CardBody>
                <div className="flex mb-3">
                    ...
                    <div className="ml-auto flex gap-3 items-center">
                        <Button
                            variant="outlined"
                            className="p-2 sm:px-6 sm:py-3 flex flex-col sm:flex-row sm:gap-1 dark:bg-blue-800 dark:hover:bg-blue-600 dark:border-gray-300 dark:border dark:text-gray-300 dark:hover:text-gray-100 dark:shadow-none"
                            onClick={() => navigate(`/block-explorer/block?block=${parsedBlockNum - 1}`)}
                        >
                            <span className="min-w-[65px] sm:min-w-0">Previous</span>
                            <span>Block</span>
                        </Button>
                        <Button
                            variant="outlined"
                            ...
                            onClick={() => navigate(`/block-explorer/block?block=${parsedBlockNum + 1}`)}
                        >
                            <span className="min-w-[65px] sm:min-w-0">Next</span>
                            <span>Block</span>
                        </Button>
                    </div>
                </div>
                            ...
                            <TableBody>
                                <TableRow>
                                    ...
                                    <TableCell className="break-all dark:border-gray-300">
                                        <Link
                                            to={`https://hivehub.dev/b/${blockNum}`}
                                            target="_blank"
                    ...
                    <List className="p-0">
                        {transactions.map((tx, i) => (
                            <React.Fragment key={tx.id}>
                                <ListItem
                                    onClick={listItemClickHandler(() => nav(`/block-explorer/transaction?id=${tx.id}`))}
                                ...
                                >
                                    <div className="pointer-events-none">
                                        ...
                                        <div className="flex flex-wrap items-center gap-2 w-full">
                                            <TxTypeChip
                                                type={tx.type}
                                                className="order-1 pointer-events-auto dark:text-gray-800 dark:bg-gray-300 dark:group-hover:bg-gray-800 dark:group-hover:text-gray-300 dark:group-focus:bg-gray-800 dark:group-focus:text-gray-300"
                                            />
                                            <AccountChip
                                                account={tx.player}
                                                className="order-4 sm:order-2 pointer-events-auto dark:text-gray-300 dark:border-gray-300 dark:group-hover:text-gray-800 dark:group-hover:border-gray-800 dark:group-focus:text-gray-800 dark:group-focus:border-gray-800"
                                            />
