...
export const legacy_price_feed = new Schema.Schema(
    ...
    object({
        ...
        // TODO: Only required because of how steemmonsters currently deals with this
        dec_price: number().positive().required(),
    }),
);
...
export class TopPriceFeedAction extends Action<typeof price_feed.actionSchema> {
    constructor(private readonly producer: PriceFeedProducer, op: OperationData, data: unknown, index?: number) {
...
class LegacyTopPriceFeedAction extends Action<typeof legacy_price_feed.actionSchema> {
    ...
    constructor(producer: PriceFeedProducer, op: OperationData, data: unknown, index?: number) {
        super(legacy_price_feed, op, data, index);
        this.proxy = new TopPriceFeedAction(producer, op, { action: this.id, params: normalizeLegacyPriceFeed(this.params) }, index);
    }
