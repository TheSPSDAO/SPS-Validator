...
beforeEach(async () => {
    ...
    await fixture.testHelper.setHiveAccount(delegatee, { delegation: [delegatorWithAuthority] });
...
test.dbOnly('Garbage data for return_tokens does not crash.', () => {
    return expect(fixture.opsHelper.processOp('return_tokens', 'steemmonsters', garbage_payload)).resolves.toBeUndefined();
});

test.dbOnly('Lots of emoji for return_tokens does not crash.', () => {
    return expect(fixture.opsHelper.processOp('return_tokens', 'steemmonsters', emoji_payload)).resolves.toBeUndefined();
});

test.dbOnly('Simple return_tokens.', async () => {
...
test.dbOnly('Simple return_tokens with authority.', async () => {
    const amount_to_undelegate = 50;
    const block_time = new Date();
    // there is an undelegation cooldown, so set this block_time beyond that.
    block_time.setTime(block_time.getTime() + 8 * 24 * 60 * 60 * 1000);
...
test.dbOnly('return_tokens without authority fails.', async () => {
...
test.dbOnly('return_tokens more than delegated amount fails.', async () => {
...
test.dbOnly('return_tokens negative amount fails.', async () => {
...
test.dbOnly('return_tokens using posting key succeeds.', async () => {
    const amount_to_undelegate = amount_delegated;
    const block_time = new Date();
    // there is an undelegation cooldown, so set this block_time beyond that.
    block_time.setTime(block_time.getTime() + 8 * 24 * 60 * 60 * 1000);
