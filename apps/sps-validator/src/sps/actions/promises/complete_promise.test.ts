...
test.dbOnly('Garbage data for complete_promise does not crash.', () => {
    return expect(fixture.opsHelper.processOp('complete_promise', promise_creator, garbage_payload)).resolves.toBeUndefined();
});

test.dbOnly('Lots of emoji for complete_promise does not crash.', () => {
    return expect(fixture.opsHelper.processOp('complete_promise', promise_creator, emoji_payload)).resolves.toBeUndefined();
});

test.dbOnly('complete_promise with invalid payload fails.', async () => {
    await expect(
        fixture.opsHelper.processOp('complete_promise', promise_creator, {
            type: 'delegation',
...
test.dbOnly('complete_promise with invalid type fails.', async () => {
    await expect(
        fixture.opsHelper.processOp('complete_promise', promise_creator, {
...
test.dbOnly('complete_promise with non-controller account fails.', async () => {
    await expect(
        fixture.opsHelper.processOp('complete_promise', delegator, {
            id: fulfilled_promise_id,
            type: 'delegation',
        }),
    ).resolves.toBeUndefined();
...
test.dbOnly('complete_promise on fulfilled promise succeeds.', async () => {
    await expect(
        fixture.opsHelper.processOp('complete_promise', promise_creator, {
            id: fulfilled_promise_id,
            type: 'delegation',
    ...
    // there is no "reveral" history for a cancelled promise that was fulfilled
    ...
    expect(delegationToSpsIn?.balance).toBe(100);
});

test.dbOnly('complete_promise on cancelled promise fails.', async () => {
    await expect(
        fixture.opsHelper.processOp('complete_promise', promise_creator, {
...
test.dbOnly('complete_promise on open promise fails.', async () => {
    await expect(
        fixture.opsHelper.processOp('complete_promise', promise_creator, {
            id: open_promise_id,
            type: 'delegation',
