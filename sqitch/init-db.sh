...
psql -Atx "${CONNECTION_STRING}" -c "SELECT 1 FROM pg_catalog.pg_namespace WHERE nspname = 'snapshot' JOIN pg_catalog.pg_roles ON pg_catalog.pg_roles.oid = pg_catalog.pg_namespace.nspowner WHERE rolname != '${APP_USER}'" |
